...
# обновляем только файлы кода (docker-compose.yml — тоже КОД: держит PER_TRADE/BANKROLL;
# update.sh — сам себя, иначе правки деплой-скрипта не доезжают. paper_book.json и пр. состояние НЕ трогаем):
git checkout -q origin/main -- '*.py' Dockerfile requirements.txt docs/index.html docker-compose.yml update.sh 2>/dev/null
