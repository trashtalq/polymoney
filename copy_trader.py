...
# НЕ копируем рынки «сколько твитов/постов напишет X за неделю» — числовые корзины (180-199, <40).
# Анализ 2026-07-12: по ним −$3.5k (99 позиций в ноль): продают премию на узкую корзину, попала —
# −100% («копейки перед катком»). 2026-07-13: добавлены POSTS (Truth Social/CZ posts пролезали мимо
# «tweets») + до 3 слов между числом и словом («140-159 Truth Social posts»). Mention не трогаем.
_TWEETCOUNT_RE = re.compile(
    r"(?:<\s*\d+|\d+\s*\+|\d+\s*[-–—]\s*\d+|\b\d+)\s*(?:\w+\s+){0,3}(?:tweets?|posts?)\b", re.I)
...
def rescale_book(book: dict, factor: float) -> dict:
    ...
    for p in book.get("positions", {}).values():
        ...
        if p.get("rcost") is not None:
            p["rcost"] = round(p["rcost"] * f, 6)
    for r in book.get("log", []):
        for k in ("spend", "pnl", "rpnl"):
            if r.get(k) is not None:
                r[k] = round(r[k] * f, 4)
    ra = book.get("realacct")
    if ra:                                                 # реал-леджер масштабируется вместе с книгой
        for k in ("base", "cash", "realized", "missed_spend"):
...
# --- Усреднение: повторяем докупки цели, режем только когда кэф УЛЕТЕЛ от нашей средней ---
MAX_POSITION_MULT = 4    # потолок позиции = 4×per_trade (усреднения допускаются, но не безгранично)
AVG_UP_TOL = 0.15        # ослаблено 0.05->0.15 (2026-07-08, решение пользователя): доверяем докупкам
                         # цели и вверх; от настоящего «улёта» защищают adverse-фильтр (цена ушла
                         # от ЕЁ входа > MAX_ADVERSE_MOVE), band (потолок 0.92) и потолок позиции
...
# Бумага живёт с автодоливами (кэш бесконечный) — реальный счёт нет. Ведём рядом жёсткий
# кэш без доливов: каждый BUY либо «влез» (кэш списан), либо «пропущен» (посчитан).
# Выходы возвращают кэш пропорционально доле владения. Даёт честный ответ:
# какую долю эджа поймает реальный капитал и сколько входов он пропустит.
# База в БУМАЖНЫХ единицах (масштаб ×10 от реала): реальный счёт ~$1756 -> 17560.
REAL_BASE = float(os.environ.get("REAL_BASE", "17560"))
...
# ----------------------------- реал-леджер (жёсткий кэш без доливов) -----------------------------
def _real_buy(book: dict, key: str, spend: float) -> bool:
    """Попытка входа на реальном кэше: влезло -> списываем и копим rcost позиции,
    не влезло -> считаем пропуск. Бинарно (без частичных входов) — чтобы статистика
    «сколько входов пропустит реальный счёт» была однозначной."""
    ra = book.setdefault("realacct", {"base": REAL_BASE, "cash": REAL_BASE, "realized": 0.0,
                                      "taken": 0, "missed": 0, "missed_spend": 0.0})
    if ra["cash"] >= spend:
        ra["cash"] -= spend
        ra["taken"] += 1
        pos = book["positions"].get(key)
        if pos is not None:
            pos["rcost"] = pos.get("rcost", 0.0) + spend
        return True
    ra["missed"] += 1
    ra["missed_spend"] += spend
    return False


def _real_exit(book: dict, pos: dict, proceeds: float, cost_part: float, rec: dict = None) -> None:
    """Выход реал-леджера той же долей, какой он владел позицией (share = rcost/cost).
    Вызывать ДО уменьшения pos.cost/qty. Пишет rpnl в запись лога (для пересборки)."""
    ra = book.get("realacct")
    rc = pos.get("rcost", 0.0) or 0.0
    cost = pos.get("cost", 0.0) or 0.0
    if not ra or rc <= 0 or cost <= 0:
        return
    share = min(1.0, rc / cost)
    pr, rpart = proceeds * share, cost_part * share
    ra["cash"] += pr
    ra["realized"] += pr - rpart
    pos["rcost"] = max(0.0, rc - rpart)
    if rec is not None:
        rec["rpnl"] = round(pr - rpart, 4)


def _rebuild_realacct(book: dict) -> None:
    """Пересборка агрегатов реал-леджера из лога/позиций (после purge_*): realized = сумма rpnl
    оставшихся закрытий, кэш = база + realized - вложенное в оставшиеся открытые (rcost)."""
    ra = book.get("realacct")
    if not ra:
        return
    base = ra.get("base", REAL_BASE)
    realized = sum((r.get("rpnl") or 0) for r in book.get("log", []))
    open_rcost = sum((p.get("rcost") or 0) for p in book.get("positions", {}).values())
    buys = [r for r in book.get("log", []) if r.get("act") == "BUY" and "r" in r]
    ra["realized"] = round(realized, 2)
    ra["cash"] = round(base + realized - open_rcost, 2)
    ra["taken"] = sum(1 for r in buys if r.get("r") == 1)
    ra["missed"] = sum(1 for r in buys if r.get("r") == 0)
    ra["missed_spend"] = round(sum((r.get("spend") or 0) for r in buys if r.get("r") == 0), 2)
...
def copy_redeem(book: dict, e: dict, wallet: str = "") -> bool:
    ...
    book["realized"] += proceeds - pos["cost"]
    book["cash"] += proceeds
    rec = {"t": ev_ts(e), "w": pos["wallet"], "act": "REDEEM", "val": 1.0,
           "pnl": round(proceeds - pos["cost"], 2), "out": pos.get("outcome", ""),
           "title": ev_title(e)[:46]}
    _real_exit(book, pos, proceeds, pos["cost"], rec)
    book["log"].append(rec)
    book["positions"].pop(key, None)
    return True
...
def settle(book: dict, wallet: str, tok: str, val: float) -> None:
    ...
    book["realized"] += proceeds - pos["cost"]
    book["cash"] += proceeds
    rec = {"t": int(time.time()), "w": pos["wallet"], "act": "SETTLE", "val": val,
           "pnl": round(proceeds - pos["cost"], 2), "out": pos.get("outcome", ""),
