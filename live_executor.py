    ...
    while True:
        ...
        for sig in r.get("signals", []):
            ...
            if allow and (sig.get("w", "") or "").lower() not in allow:
                # ДВА РАЗНЫХ СОБЫТИЯ, и путать их нельзя. Убранный из состава кошелёк остаётся в
                # ленте, пока мы держим его позиции (см. hold_ws выше) — иначе мы не поймаем его
                # ВЫХОДЫ и унаследованные позы зависнут до резолва. Это штатный отказ от нового
                # входа, а не признак атаки. Кричать «подсунут» надо только про кошелёк, которого
                # мы вообще не знаем: вот это уже похоже на подмену ленты.
                _w = (sig.get("w", "") or "").lower()
                _known = _w in {w for w in (state.get("tok_wallet") or {}).values() if w}
                why = ("убран из состава, держим его позиции — новые входы не берём"
                       if _known else "НЕИЗВЕСТНЫЙ кошелёк — сигнал мог быть подсунут")
                print(f"  пропуск: {_w[:10]}… {why} — {title[:36]}", flush=True)
                state["done"].append(k)
                continue
            # --- гварды ---
