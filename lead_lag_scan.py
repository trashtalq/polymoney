...
MIN_EARLY_USD = 75       # минимум $ ранних покупок кандидата в рынке
                         # 2026-07-11: 50->75 — форвард-винрейт добычи (42%, n=332 закрытых)
                         # заметно ниже прочих источников системы (wide-скан 54%, скаут-macro 53%,
                         # со-трейдер 58%); MIN_HITS/sim_pnl при этом НЕ коррелируют с форвардом
                         # (bucket hits=4 — лучший, не худший), поднимать его не за что — поднимаем
                         # порог конвикшена в $, это независимая от hits ось фильтра шума
